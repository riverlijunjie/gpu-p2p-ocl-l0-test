...
void run_queue_sync_test(bool is_event_sync_mode, size_t bytes_size, int loop)
{
    int iters = 100;
    ...
    {
        std::cout << "loop = " << loop << ", iter = " << i << std::endl;
        ...
        if (is_event_sync_mode)
        {
            ...
            clFinish(queue_2);
            clReleaseMemObject(shared_mem); // // MUST be released
        }
        else
        {
            clReleaseMemObject(shared_mem); // MUST be released
        ...
        if(g_hit==0 && cnt !=0) {
            g_hit = loop*100 + iters;
        }

        if (is_event_sync_mode)
        {
            printf("Use event to sync queues between different gpu devices: %s(%.8f), g_hit = %d\n\n", cnt == 0 ? "SUCCESS" : "FAIL", 1.0 * cnt / size, g_hit);
        }
        else
        {
            printf("Use clFinish to sync queues between different gpu devices: %s(%.8f), g_hit = %d\n\n", cnt == 0 ? "SUCCESS" : "FAIL", 1.0 * cnt / size, g_hit);
        }

        clReleaseMemObject(bufA);
        // clReleaseMemObject(shared_mem); // must release, else OOM
...
int main(int argc, char **argv)
{
    ...
    for(int i = 0; i < cnt; i++) {
        ...
        run_queue_sync_test(0, bytes, i); // manual sync(clFinish) will sucess
        run_queue_sync_test(1, bytes, i); // cl_event sync will fail
    }
