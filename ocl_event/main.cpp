...
void run_queue_sync_test(bool is_event_sync_mode, size_t bytes_size, cl_device_id device_1, cl_device_id device_2)
{
    ...
    clGetDeviceInfo(device_1, CL_DEVICE_EXTENSIONS, 0, NULL, &extensions_size);
    char *extensions = (char *)malloc(extensions_size);
    clGetDeviceInfo(device_1, CL_DEVICE_EXTENSIONS, extensions_size, extensions, NULL);
    std::cout << "extensions: " << extensions << std::endl;
    if (strstr(extensions, "cl_intel_command_queue_families"))
    {
        std::cout << "Support cl_intel_command_queue_families: TRUE" << std::endl;
    }

    size_t family_size, ret_size;
    clGetDeviceInfo(device_1, CL_DEVICE_QUEUE_FAMILY_PROPERTIES_INTEL, 0, NULL, &family_size);
    int number = family_size/sizeof(cl_queue_family_properties_intel);
    cl_queue_family_properties_intel *families = static_cast<cl_queue_family_properties_intel *>(malloc(family_size));
    clGetDeviceInfo(device_1, CL_DEVICE_QUEUE_FAMILY_PROPERTIES_INTEL, family_size, families, &ret_size);
    for(int i = 0; i< number;i++){
        std::cout << "cl_queue_family_properties_intel[" << i << "]:" << std::endl;
        std::cout << "       properties = " << std::bitset<32>(families[i].properties) << std::endl;
        std::cout << "       capabilities = " << std::bitset<32>(families[i].capabilities) << std::endl;
        std::cout << "       count = " << families[i].count << std::endl;
        std::cout << "       name = " << families[i].name << std::endl;
    }
    std::cout << std::endl;

    cl_command_queue_properties props_1[] = {CL_QUEUE_PROPERTIES, 0,
                                             CL_QUEUE_FAMILY_INTEL, 1,
                                             CL_QUEUE_INDEX_INTEL, 0,
                                             // CL_DEVICE_QUEUE_FAMILY_PROPERTIES_INTEL,
                                             // CL_QUEUE_CAPABILITY_CREATE_CROSS_QUEUE_EVENTS_INTEL | CL_QUEUE_CAPABILITY_TRANSFER_BUFFER_INTEL,
                                             0};
    cl_command_queue queue_1 = clCreateCommandQueueWithProperties(context_1, device_1, props_1, &err);
    CHECK_OCL_ERROR_EXIT(err, "clCreateCommandQueueWithProperties 1 failed");
    cl_command_queue_properties props_2[] = {CL_QUEUE_PROPERTIES, 0,
                                             CL_QUEUE_FAMILY_INTEL, 1,
                                             CL_QUEUE_INDEX_INTEL, 0,
                                             // CL_DEVICE_QUEUE_FAMILY_PROPERTIES_INTEL,
                                             // CL_QUEUE_CAPABILITY_CROSS_QUEUE_EVENT_WAIT_LIST_INTEL,
                                             0};
    cl_command_queue queue_2 = clCreateCommandQueueWithProperties(context_2, device_2, props_2, &err);
    CHECK_OCL_ERROR_EXIT(err, "clCreateCommandQueueWithProperties 2 failed");
    ...
    uint64_t fd;
    err = clGetMemObjectInfo(bufB, CL_MEM_ALLOCATION_HANDLE_INTEL, sizeof(fd), &fd, NULL); CHECK_OCL_ERROR_EXIT(err, "clGetMemObjectInfo failed");
...
{
    int cnt = 1;
    int bytes = 128 * 1024 * 1024; //129MB
    if (argc == 3)
    {
        cnt = atoi(argv[1]);   // iterations
        bytes = atoi(argv[2]); // MB
        bytes *=1024*1024;
    }
    ...
    for(int k = 0; k < cnt; k++) {
        ...
        run_queue_sync_test(0, bytes, device_2, device_1);
